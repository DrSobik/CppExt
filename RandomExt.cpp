...
namespace Common {
    ...
    void GeneralRandGen::setSeeds(const QList<quint32>& seeds) {
        this->seeds = seeds;
        init();
    }

    QList<quint32> GeneralRandGen::getSeeds() {
        return this->seeds;
    }
    ...
    quint32 GeneralRandGen::rndInt(const quint32& a, const quint32& b) {
        ...
        //out << "GeneralRandGen::rndInt(const quint32& a, const quint32& b) " << a << "," << b << ", " << rndInteger << endl;

        quint32 res = a + (quint32) qRound64(double (rndInteger) / double (maxGenInt) * double (b - a));

        //out << "GeneralRandGen::rndInt(const quint32& a, const quint32& b) result " << res << endl;
    ...
    GeneralRandGen::GeneralRandGen() : maxGenInt(quint32((quint64(2) << 31) - 1)), QObject(0) {

    }

    GeneralRandGen::GeneralRandGen(const GeneralRandGen& orig) : maxGenInt(quint32((quint64(2) << 31) - 1)), QObject(0) {
        seeds = orig.seeds;
    }

    GeneralRandGen::GeneralRandGen(const QList<quint32>& seeds) : maxGenInt(quint32((quint64(2) << 31) - 1)), QObject(0) { // Initialize the generator
        //Debugger::info << "GeneralRandGen::RandGen : Creating!" << ENDL;
    }
    ...
    double GeneralRandGen::rndFloat() {
        return ((double) rndInt()) / double(maxGenInt);
    }
    ...
    RandGenMT::RandGenMT(const RandGenMT& orig) : GeneralRandGen(orig) {
        for (int i = 0; i < 624; i++) MT[i] = orig.MT[i];
    ...
    RandGenMT::RandGenMT(const QList<quint32>& seeds) : GeneralRandGen(seeds) { // Initialize the generator
        setSeeds(seeds);
    }
    ...
    RandGenLCGNL::RandGenLCGNL(const RandGenLCGNL& orig) : GeneralRandGen(orig), a(quint64(6364136223846793005)), c(quint64(1)), m(quint64(quint64(2) << 63) - 1) {
        X = orig.X;
    }
    ...
    RandGenLCGNL::RandGenLCGNL(const QList<quint32>& seeds) : GeneralRandGen(seeds), a(quint64(6364136223846793005)), c(quint64(1)), m(quint64(quint64(2) << 63) - 1) {
        setSeeds(seeds);
    }
